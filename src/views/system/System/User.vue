        ...
        <div class="table-header">
          <a-form layout="inline" :model="searchForm" class="search-form">
            <a-form-item label="用户名">
              <a-textarea v-model:value="searchForm.username" placeholder="请输入用户名" :rows="5" allow-clear />
            </a-form-item>
...
const fetchData = async () => {
  loading.value = true
  try {
    const res: any = await mockApi.getUsers({
...
const calculateTableHeight = () => {
  nextTick(() => {
    setTimeout(() => {
      const tableWrapper = document.querySelector('.user-management .table-wrapper')
